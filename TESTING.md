...
This doc proposes the structure. Nothing here is wired up yet — see the **Adoption plan** (§8)
for the order we introduce it (start now, at M1, while the surface is small).
...
Add a task runner (recommend **`just`** — a simple `justfile`) so `just lint` / `just test` /
`just ci` work identically locally and in CI:
...
| large output burst (`yes | head -c 1MB`) → no loss/hang | backpressure |
...
> what E2E is for. Test *wiring* here, *behavior* in E2E.
...
| When | Add |
|------|-----|
| **Now (before M1 code)** | `rustfmt.toml`, clippy `-D warnings` in a basic CI; strict `tsconfig`; ESLint + Prettier; `justfile`; lefthook pre-commit. **Plus the first PTY integration test** for the M0 bridge (spawn+echo+kill) — lock in the core before building on it. |
