...
| **6e**    | **Files browser** — lazy per-folder `readdir` rooted at the focused pane's cwd (click a folder → one `ls`, never a recursive walk); click a file → open. Right-rail panel; reuses the `openFile` IPC     | fs `readdir` IPC           | low                                              |
| **6f**    | **Worktree + branch on session nodes** — derive from each session's cwd (`git rev-parse --show-toplevel` + branch, cached); honor `WorktreeCreate/Remove` hooks. Sub-agents share the session's cwd      | `git.ts`                   | low                                              |
